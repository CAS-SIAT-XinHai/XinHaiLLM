...
class Simulation:
            ...
            if(agent_type=="verify_agent"):
                agent = AGENT_REGISTRY[agent_type](**agent_configs,
                                               ...
                                               environment_id=env_config["environment_id"],
                                                user_question=env_config['user_question']
                                                )
            elif(agent_type=="mllm_agent"):
                agent = AGENT_REGISTRY[agent_type](**agent_configs,
                                                   controller_address=env_config['controller_address'],
                                                   environment_id=env_config["environment_id"],
                                                  user_question = env_config['user_question']
    ...
    def run(self):
        ...
        #print(json.loads(json.dumps(self.agents[0].memory.short_term_memory.model_dump_json(), indent=2)))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config_path", type=str, default="/examples/OCRAgencyV1/configs/xinhai_ocr_V1.yaml")
