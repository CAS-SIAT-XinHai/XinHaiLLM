...
def get_db(paths, MODEL_PATH, CHROMA_PATH):
    ...
    for path in paths:
        ...
        # construct documents with "id" metadata
        for indx, doc in enumerate(documents):
            doc.metadata["id"] = ids[indx]
        if not check_db(CHROMA_PATH):
            # 判断是否需要分批添加文档
            if len(documents) > batch_size_limit:
                batch = documents[0:batch_size_limit]
                batch_ids = ids[0:batch_size_limit]
                db = Chroma.from_documents(batch, ids=batch_ids, embedding=embeddings, persist_directory=CHROMA_PATH)
                for i in range(batch_size_limit, len(documents), batch_size_limit):
                    batch = documents[i:min(i + batch_size_limit, len(documents))]
                    batch_ids = ids[i:min(i + batch_size_limit, len(documents))]
                    db.add_documents(documents=batch, ids=batch_ids)
            else:
                db = Chroma.from_documents(documents, ids=ids, embedding=embeddings, persist_directory=CHROMA_PATH)
        else:
            # 判断是否需要分批添加文档
