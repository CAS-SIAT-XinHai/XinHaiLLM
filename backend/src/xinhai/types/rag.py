...
class XinHaiRAGIndexerTypes(str, Enum):
    FAISS = 'faiss'
    CHROMA = 'chroma'
    LUCENE = 'lucene'


class XinHaiRAGDocumentIn(BaseModel):
    id: str
    text: str
    metadata: dict = {}


class XinHaiRAGDocumentOut(BaseModel):
    document: XinHaiRAGDocumentIn
    score: float


class XinHaiRAGRetrievedResult(BaseModel):
    query: str
    num: int
    documents: List[XinHaiRAGDocumentOut]


class XinHaiRAGRetrieverTypes(str, Enum):
    BM25 = 'bm25'
    DENSE = 'dense'
    NOOP = 'noop'
...
class XinHaiRAGGeneratorTypes(str, Enum):
    API = 'API'
    HF = 'huggingface'


class XinHaiRAGMethodTypes(str, Enum):
...
@dataclass
class XinHaiRAGPromptTypeMixin:
    prompt_name: str
    description: Dict[XinHaiI18NLocales, List[str]]


class XinHaiRAGPrompt(XinHaiRAGPromptTypeMixin, Enum):
    SELF_ASK_PROMPT_SINGLE_HOP = "SELF_ASK_PROMPT_SINGLE_HOP", {
        XinHaiI18NLocales.CHINESE: """""",
        XinHaiI18NLocales.ENGLISH: """Given the following question, answer it by providing follow up questions and intermediate answers. If intermediate questions are not necessarry, answer the question directly. You are provided with evidence that can help you arrive at the answer before the question.
#
...
#"""
    ...
    SELF_ASK_PROMPT_MULTI_HOP = "SELF_ASK_PROMPT_MULTI_HOP", {
        XinHaiI18NLocales.CHINESE: """""",
        XinHaiI18NLocales.ENGLISH: """Given the following question, answer it by providing follow up questions and intermediate answers. If intermediate questions are not necessarry, answer the question directly. You are provided with evidence that can help you arrive at the answer before the question.
#
