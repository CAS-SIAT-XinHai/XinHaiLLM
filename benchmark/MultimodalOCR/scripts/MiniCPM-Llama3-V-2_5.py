...
project_root = '/home/wuhaihong/xinhai/MultimodalOCR'
...
def _get_args():
    ...
    parser.add_argument("--model_path", type=str, default="/data/pretrained_models/MiniCPM-Llama3-V-2_5")#TODO Set the address of your model's weights
    parser.add_argument("--save_name", type=str, default="MiniCPM-Llama3-V-2_5") #TODO Set the name of the JSON file you save in the output_folder.
...
def eval_worker(args, data, eval_id, output_queue):
    ...
    model = AutoModel.from_pretrained(checkpoint, trust_remote_code=True, torch_dtype=torch.float16)
    model = model.to(device=f'cuda:{eval_id}')
    ...
    for i in tqdm(range(len(data))):
        ...
        data[i]['predict'] =res
    output_queue.append({eval_id: data})
...
if __name__=="__main__":
    ...
    # output_queue = []
    # for i in range(len(data_list)):
    #     result = eval_worker(args, data_list[i], i, output_queue)
